...
public class ArbolDirectorio {
	...
	public int size() {
		int cont = 0;
        Carpeta n = root;
        for (Carpeta h : n.hijos) {
            cont++;
            for (Carpeta z : h.hijos) {
                cont++;
            }
        }
        return cont;
	...
	public boolean mover(String path1, String path2) {
        if (ir(path1) != null && ir(path2) != null) {
			if (ir(path1).hijos != ir(path2).hijos) {
				ir(path2).hijos.add(new Carpeta(path1));
			}
            // while (ir(path1).hijos.element() != ir(path2).hijos.element()) {
            //     ir(path2).hijos.add(new Carpeta(path1).hijos.element());
            // }
